...
pchibarsq=function(q, V, wts, lower.tail=TRUE, log.p=FALSE)
{
  n<-nrow(V)
...
{## Ref: Kudo A. 1963. Biometrika, 50, pg 403  (page 414)
  ...
  ans[1]<-pmvnorm(rep(0,n),rep(Inf,n),sigma=solve(V))[[1]]
  ans[n+1L]<-pmvnorm(rep(0,n), rep(Inf,n), sigma=V)[[1]]
  for(i in seq(1L, n-1L, by=1L)) ans[i+1]= (2^(-n)*factorial(n))/(factorial(i)*factorial(n-i))
  ans
}
...
qchibarsq =function(p, V)
{
  K = ncol(V)
  wts<-wchibarsq(V)
  pchiroot = function(q, V, wts){pchibarsq(q, V, wts) - p}
