################################################################################
#                     For PAIRED experiments computes the maximal 
...
maxCorPair = function(Q, index, Gamma)
{
  ...
  CorPairMax = function(pprod, qnew, index)
  {
  ...
  GradCorPairMax = function(pprod, qnew, index)
  {
  ...
  -optim(pprod, CorPairMax, gr = GradCorPairMax, qnew=qnew, index=index, method = "L-BFGS-B", lower = Gamma/(1+Gamma)^2, upper = 1/4, control = list(lmm = 5, pgtol = 1e-8, fnscale = 1e-4))$value
