...
#crossscreen proportion
planning_sample_prop=0.5
#divide data into planning and analysis samples
planning_sample_size_sets <- floor(planning_sample_prop*nostratum)
ix1 <- sort(sample(x=1:nostratum,size=planning_sample_size_sets))
ix2 <- (1:nostratum)[-ix1]
...
sv_whole <- sv_planning <- sv_analysis <- sv_analysis_withsearch <- sv_whole_planreused <- 
  ...
  sv_whole_designsens <- sv_analysis_designsens <- sv_crossscreen <- numeric(length=nsim)
...
for (sim in 1:nsim){
  ...
  ### SAMPLE 1###
  Data_1 = syntheticData$normalData #this can be changed to use t-noised data
  Data_1$YData <- Data_1$YData[ix1,]
  Data_1$s_k = apply(abs(Data_1$YData), MARGIN = 2, FUN = median)  # s_k is set to the median currently
  psi_1 = psi_huber #this can be changed to use an inner-trimmed psi function
  
  experimentalSetup_1 = makeExperimentalSetupFromYData(Data_1, psi_1)
  Q_1 = experimentalSetup_1$Q
  populationSize_1 = dim(Q_1)[2]
  K_1 = dim(Q_1)[1]
  Z_1 = experimentalSetup_1$Z #the treatment indicator
  index_1 = experimentalSetup_1$index
  
  matchedSetAssignments_1 = rep(0, populationSize_1)
  for(ind in 1:length(index_1))
  {
    matchedSetAssignments_1[unlist(index_1[ind])] = ind
  }
  
  
  ### SAMPLE 2###
  Data_2 = syntheticData$normalData #this can be changed to use t-noised data
  Data_2$YData <- Data_2$YData[ix2,]
  Data_2$s_k = apply(abs(Data_2$YData), MARGIN = 2, FUN = median)  # s_k is set to the median currently
  psi_2 = psi_huber #this can be changed to use an inner-trimmed psi function
  
  experimentalSetup_2 = makeExperimentalSetupFromYData(Data_2, psi_2)
  Q_2 = experimentalSetup_2$Q
  populationSize_2 = dim(Q_2)[2]
  K_2 = dim(Q_2)[1]
  Z_2 = experimentalSetup_2$Z #the treatment indicator
  index_2 = experimentalSetup_2$index
  
  matchedSetAssignments_2 = rep(0, populationSize_2)
  for(ind in 1:length(index_2))
  {
    matchedSetAssignments_2[unlist(index_2[ind])] = ind
  }
  ...
  ### SAMPLE1 ###
  sensitivityResult_1old = chiBarSquaredTest(Q = Q_1, #the data matrix
                                              matchedSetAssignments = matchedSetAssignments_1, #the stratum numbers for the individuals
                                              treatmentIndicator = Z_1, #the treatment indicator
                                              numGamma = numGamma, #the number of Gammas to try
                                              alpha = .05, #the significance level of the test
                                              directions = directions, #the directions of each hypothesis
                                              step = 10, #optimization hyperparameter
                                              ...
                                              outputDirName = "SyntheticExample_Sensitivity_Analysis_Results_Whole")
  ### SAMPLE2 ###
  sensitivityResult_2old = chiBarSquaredTest(Q = Q_2, #the data matrix
                                          matchedSetAssignments = matchedSetAssignments_2, #the stratum numbers for the individuals
                                          treatmentIndicator = Z_2, #the treatment indicator
                                          numGamma = numGamma, #the number of Gammas to try
                                          alpha = .05, #the significance level of the test
                                          directions = directions, #the directions of each hypothesis
                                          step = 10, #optimization hyperparameter
  ...
  ### SAMPLE1 ###
  sensitivityResult_1 = chiBarSquaredTest(Q = Q_1, #the data matrix
                                          matchedSetAssignments = matchedSetAssignments_1, #the stratum numbers for the individuals
                                          treatmentIndicator = Z_1, #the treatment indicator
                                          numGamma = numGamma, #the number of Gammas to try
                                          alpha = .05/2, #the significance level of the test
                                          directions = directions, #the directions of each hypothesis
                                          step = 10, #optimization hyperparameter
                                          maxIter = 1000, #optimization hyperparameter
                                          showDiagnostics = showDiagnostics, #whether or not diagonstics are output
                                          verbose = verbose,
                                          outputDirName = "SyntheticExample_Sensitivity_Analysis_Results_Whole",
                                          lam_init = sensitivityResult_2old$OptLambda)
  ### SAMPLE2 ###
  sensitivityResult_2 = chiBarSquaredTest(Q = Q_2, #the data matrix
                                          matchedSetAssignments = matchedSetAssignments_2, #the stratum numbers for the individuals
                                          treatmentIndicator = Z_2, #the treatment indicator
                                          numGamma = numGamma, #the number of Gammas to try
                                          alpha = .05/2, #the significance level of the test
                                          ...
                                          verbose = verbose,
                                          outputDirName = "SyntheticExample_Sensitivity_Analysis_Results_Whole",
                                          lam_init = sensitivityResult_1old$OptLambda)
  
  
  crossscreenix = which.max(c(sensitivityResult_1$LargestRejectGamma,sensitivityResult_2$LargestRejectGamma))
  crossscreenres = if(crossscreenix==1) sensitivityResult_1 else sensitivityResult_2
  ...
  res=list(
    ...
    sensitivityResult_crossscreen=crossscreenres
  )
  ...
  sv_crossscreen[sim] <- as.numeric(sensvals[7])
}


gammas_vector <- seq(from=1,to=100,by=0.25)
...
power_sv_crossscreen <- colSums(outer(sv_crossscreen, gammas_vector, `>`))/nsim
...
methodnames <- c('Analysis w/ Planning','Whole w/ Whole',
                 'Analysis w/ Analysis', 'Whole w/ Max DesSens', 'Analysis w/ Max DesSens', 'Cross Screening')
df <- data.frame(Gamma=rep(gammas_vector,length(methodnames)),
